...
/**
 * Fixed-point arithmetic for money amounts, at 8 decimal places (10^8 scaling).
 * All balances/holds/amounts are stored as {@code long} to avoid BigDecimal allocations.
 *
 * <p>Copied verbatim from the matching engine ({@code com.match.domain.FixedPoint}) so the
 * Assets Engine shares the exact money semantics (same rounding, same overflow behaviour) while
 * staying a self-contained bounded context with no cross-module dependency.</p>
 */
public final class FixedPoint {
    ...
    /**
     * Thrown when a fixed-point result cannot be represented in a signed 64-bit value.
     * Wrong money must never be returned silently: callers on money-mutation paths catch
     * this and reject. The stack trace is suppressed so throwing stays cheap on the engine
     * thread under hostile input.
     */
    ...
    /**
     * Multiply two fixed-point values exactly (result in fixed-point, truncated toward zero).
     *
     * Path selection is by whether the PRODUCT fits in 64 bits — never by operand magnitude,
     * so argument order does not matter. The wide path is an exact 128/64 division.
     *
    ...
    /**
     * Divide two fixed-point values exactly (result in fixed-point, truncated toward zero):
     * (numerator * SCALE_FACTOR) / denominator with a 128-bit intermediate.
     *
    ...
    /**
     * Unsigned 128-by-64-bit division returning the 64-bit quotient (Hacker's Delight 9-4,
     * divlu). Caller guarantees {@code hi < divisor} unsigned; divisor must be nonzero.
     */
    ...
    /**
     * Format fixed-point value as a decimal string with exactly {@link #SCALE} fractional
     * digits (the canonical wire representation: {@code parse(format(x)) == x} for every long).
     */
    ...
    /**
     * Parse a decimal string into fixed-point, exactly. Grammar: {@code -?digits[.digits]}
     * with 1 to {@link #SCALE} fractional digits. Anything else throws — money precision is
