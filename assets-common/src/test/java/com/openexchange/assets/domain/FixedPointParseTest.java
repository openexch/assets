...
//
// PORTED FROM match-common. FixedPoint is duplicated: the two copies are ~100%
// identical (only javadoc formatting differs) and the match#30 overflow fix was
// applied to both BY HAND.
//
// These tests existed only on the matching engine's copy, which meant the MONEY
// LEDGER's arithmetic ran with no guard at all. That is the more dangerous half
// of a duplicated file: not that the copies differ today, but that one of them
// is unwatched, so the day they do differ nothing says so.
//
// This closes the gap cheaply. The real fix is one FixedPoint in a shared
// artifact, the way the cluster machinery went into openexch/cluster-kit; until
// then a change to either copy must be made to both, and to both test suites.
